...
async fn main(spawner: Spawner) {
    ...
    #[cfg(feature = "power_sensors")]
    {
        ...
        let i2c_manager = {
            for addr in bsp::consts::INA226_ADDRS {
                let mut dummy = [0u8; 0];
                let dummy2 = [0u8; 0];
                if sensors_i2c.blocking_write_read(addr, &dummy2, &mut dummy).is_ok() {
                    // Device found
                }
            }
        ...
        static POWER_SENSORS: OnceLock<Mutex<CriticalSectionRawMutex, (
    ...
    #[allow(dead_code)]
    static CAN_IFACE: OnceLock<can::BufferedCan<'static, CAN_TX_BUF_SIZE, CAN_RX_BUF_SIZE>> = OnceLock::new();
    CAN_IFACE.get_or_init(|| can_iface);
    
    static MAIN_ERROR_SENDER: OnceLock<Mutex<CriticalSectionRawMutex, crate::bsp::BufferedCanErrorSender>> = OnceLock::new();
    MAIN_ERROR_SENDER.get_or_init(|| Mutex::new(error_sender.clone()));

    // Setup event channel
    static MAIN_CHANNEL: StaticCell<embassy_sync::channel::Channel<CriticalSectionRawMutex, MainEvent, MAIN_EVENT_CAPACITY>> =
        StaticCell::new();
    ...
    *(NVSTORE.nv.lock().await) = Some(nvs);
    ...
    static APP: OnceLock<Mutex<CriticalSectionRawMutex, crate::application::MonitorApp>> = OnceLock::new();
...
async fn init_main_task() {
    ...
    #[cfg(feature = "power_sensors")]
    {
        ...
        match unlocked.monitors.setup(0, embassy_time::Instant::now()).await {
            Err(e) => {
    ...
    loop {
        static EVENT_CHANNEL: StaticCell<embassy_sync::channel::Channel<CriticalSectionRawMutex, MainEvent, MAIN_EVENT_CAPACITY>> =
            StaticCell::new();
        let main_channel = EVENT_CHANNEL.init(embassy_sync::channel::Channel::new());
...
async fn init_storage(shared_nvs: &nvstore::SharedNvStore) -> Result<(), crate::error::Error> {
    ...
    let nvs = unlocked.as_mut().ok_or(crate::error::mkerr(FILE_CODE, crate::ErrorCode::NoDevice, line!()))?;
    nvs.init().await?;
    
    #[cfg(feature = "power_sensors")]
    {
        let _ret = nvs.load_last_monitor_observation(&APPDATA, -1, &mut nvstore::power_sensors::Header::new()).await;
    }
    Ok(())
}
        ...
        match select3(
            ...
            Either3::First(_) => {
                if enc_a.is_high().unwrap_or(false) {
                    if encoder_state == EncoderState::B {
                        ...
                        let _ = sender.send(MainEvent::HID(HumanEvent::EncoderAntiClockwise)).await;
                    } else {
            ...
            Either3::Second(_) => {
                if enc_b.is_high().unwrap_or(false) {
                    if encoder_state == EncoderState::A {
                        encoder_state = EncoderState::Idle;
                        let _ = sender.send(MainEvent::HID(HumanEvent::EncoderClockwise)).await;
                    } else {
            ...
            Either3::Third(_) => {
                if button.is_high().unwrap_or(false) {
                    ...
                    let _ = sender.send(MainEvent::HID(HumanEvent::EncoderButtonPressed)).await;
                } else {
                    if let Some(start) = press_time.take() {
                        let duration = embassy_time::Instant::now() - start;
                        let _ = sender.send(MainEvent::HID(HumanEvent::EncoderButtonReleased(duration))).await;
                    }
                }
            },
        }
...
async fn ignition_task(mut cansleep: embassy_stm32::gpio::Output<'static>) {
    ...
    #[cfg(not(feature = "power_sensors"))]
    {
        ...
        loop { embassy_time::Timer::after(embassy_time::Duration::from_millis(1000)).await; }
    }
}
        ...
        for idx in 0..devices.len() {
            match devices[idx].chip.die_id().await {
                Ok(_id) => {
                    ...
                    devices[idx].chip.set_mask_enable(ina226::MaskEnableFlags::CNVR).await.unwrap();
                }
                Err(_e) => {
...
async fn run_power_sensors(
    devices: &mut MonitorInterfaces,
    alert_pins: &bsp::MonitorAlertPins,
) {
    ...
    loop {
        ...
        let f0 = unsafe { (&mut *(pins_ptr as *mut embassy_stm32::exti::ExtiInput<'static>)).wait_for_low() };
        let f1 = unsafe { (&mut *((pins_ptr as *mut embassy_stm32::exti::ExtiInput<'static>).add(1))).wait_for_low() };
        let f2 = unsafe { (&mut *((pins_ptr as *mut embassy_stm32::exti::ExtiInput<'static>).add(2))).wait_for_low() };
        let f3 = unsafe { (&mut *((pins_ptr as *mut embassy_stm32::exti::ExtiInput<'static>).add(3))).wait_for_low() };
...
async fn read_monitor(chip: &mut MonitorChip, index: u8) {
    ...
    match (bus, shunt) {
        (Ok(bus), Ok(shunt)) => {
            ...
            CAN_SUSPENDED_SENDER.get().await.send(MainEvent::MonitorObservation(obs)).await;
        }
...
static IGNITION_PIN: OnceLock<Mutex<CriticalSectionRawMutex, bsp::ExtiPin>> = OnceLock::new();
#[allow(dead_code)]
static CAN_SUSPENDED_SENDER: OnceLock<Sender<'static, CriticalSectionRawMutex, MainEvent, MAIN_EVENT_CAPACITY>> = OnceLock::new();
...
#[cfg(feature = "terminal")]
static ENCODER_ARGS: OnceLock<Mutex<CriticalSectionRawMutex, (
    crate::bsp::InputPins,
